$(window).on('load',function(e){
	...
	var checkOpen = $('#textData').val();
	if(checkOpen=='7b7b34345d77bf183a1f960594c40d87daa7c583'){
		$("#content-profile").load("productData.php");

		$("#voucher-link").attr("class","menu-waves-block");
	...
	else if(checkOpen=='7ba7afaf73ee78cf7e0c210b49b7ce3be26a04e9'){
    ...
    $.ajax({
     	type: "GET",
     	url: "../../../conn/data2/notification/selectNotifAdmin.php/?notificationShippedAdmin",
     	contentType: 'application/json; charset=utf-8',
        dataType: 'json',
     	success: function(notificationShippedAdmin) { 
 			var DataComment = jQuery.parseJSON(notificationShippedAdmin);
 			var notificationShippedAdmin = DataComment[0].call_data_notifikasi_shipped;
 			if(notificationShippedAdmin!=0){
				$('#shippedProduct').text(notificationShippedAdmin);
				$('#shippedProduct').attr('style','display:block;');
			}
 			else{
 				$('#shippedProduct').text('');
				$('#shippedProduct').attr('style','display:none;');
 			}
 		}
 	});
...
$('#voucher-link').on('click',function(e){
	$("#content-profile").load("voucherData.php");
	...
	$("#voucher-link").attr("class","active-menu-right");
});
...
$('#testimoni-link').on('click',function(e){
	$("#content-profile").load("testimoniData.php");
